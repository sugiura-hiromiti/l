use crate::l::br_exercise::js::dom::AttrMap;
use crate::l::br_exercise::js::dom::Element;
...
where
	...
	Input::Error: ParseError<Input::Token, Input::Range, Input::Position>,
{
	attempt(many(choice((attempt(element()), attempt(text()))).skip(whitespaces())))
}
...
where
	...
	Input::Error: ParseError<Input::Token, Input::Range, Input::Position>,
{
	(open_tag().skip(whitespaces()), nodes().skip(whitespaces()), close_tag()).and_then(
	...
	Input::Error: ParseError<Input::Token, Input::Range, Input::Position>,
{
	...
	let open_tag_content =
		(open_tag_name, many::<String, _, _>(space().or(newline())), attributes())

use crate::l::br_exercise::js::dom::Node;
use crate::l::br_exercise::js::dom::NodeType;
use crate::l::br_exercise::js::dom::Text;
use crate::l::br_exercise::js::renderer::Renderer;
use cursive::CbSink;
...
pub struct JavaScriptRuntimeState {
	...
	pub renderer_api:     Rc<RendererAPI,>, //Reference to browser's API
	pub document_element: Rc<RefCell<Box<Node,>,>,>, //Reference to DOM tree
}
...
impl JavaScriptRuntime {
	pub fn new() -> Self {
		...
		isolate.set_slot(Rc::new(RefCell::new(JavaScriptRuntimeState {
			context,
			renderer_api: Rc::new(RendererAPI::new(Rc::new(
				cursive::Cursive::new().cb_sink().clone(),
			),),),
			document_element: Rc::new(RefCell::new(Box::new(Node {
				node_type: NodeType::Text(Text { data: "".to_string(), },),
				children:  vec![],
			},),),),
		},),),);
	...
	pub fn execute(&mut self, filename: &str, source: &str,) -> Result<String, String,> {
		...
		match script.run(&mut tc_scope,) {
			Some(result,) => {
				Ok(result.to_string(&mut tc_scope,).unwrap().to_rust_string_lossy(&mut tc_scope,),)
			},
			None => {
				assert!(tc_scope.has_caught());
...
impl JavaScriptRuntime {
	...
	pub fn state(isolate: &v8::Isolate,) -> Rc<RefCell<JavaScriptRuntimeState,>,> {
		let s = isolate.get_slot::<Rc<RefCell<JavaScriptRuntimeState,>,>>().unwrap();
		s.clone()
	}
...
impl RendererAPI {
	...
	//request update rendering to Cursive view
	pub fn rerender(&self,) {
		self.ui_cb_sink
			.send(Box::new(move |s: &mut cursive::Cursive| {
				let screen = s.screen_mut();
				let layer: &mut Renderer =
					screen.get_mut(LayerPosition::FromFront(0,),).unwrap().downcast_mut().unwrap();
				layer.rerender()
			},),)
			.unwrap();
	}
...
fn to_pretty_string(mut try_catch: v8::TryCatch<v8::HandleScope,>,) -> String {
	...
	let filename = message.get_script_resource_name(&mut try_catch,).map_or_else(
		|| "(unknown)".into(),
		|s| s.to_string(&mut try_catch,).unwrap().to_rust_string_lossy(&mut try_catch,),
	);
